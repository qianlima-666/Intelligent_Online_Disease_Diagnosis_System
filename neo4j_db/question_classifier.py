...
class QuestionClassifier:
    def __init__(self):
        ...
        logger.info('模型正在初始化，请稍等')
        dict_directory = config["neo4j_db"]["dict_directory"]
        cur_dir = os.getcwd() + '/' + dict_directory + '/'
        #　特征词路径
        self.disease_path = os.path.join(cur_dir, 'disease.txt')
        self.department_path = os.path.join(cur_dir, 'department.txt')
        self.check_path = os.path.join(cur_dir, 'check.txt')
        self.drug_path = os.path.join(cur_dir, 'drug.txt')
        self.food_path = os.path.join(cur_dir, 'food.txt')
        self.producer_path = os.path.join(cur_dir, 'producer.txt')
